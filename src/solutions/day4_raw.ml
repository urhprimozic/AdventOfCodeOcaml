let test_str = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
let flip = 
    ...
    repna []
let get_data data=
    let rec repna (acc : string list) (tmp: string) = function
        | [] -> tmp::acc
        | x::xs -> match x with
            | "" -> repna (tmp::acc) ("") xs (* grozno. Boljš bi blo tmp::acc, ampak to zahteva od mene, da spišem še rotacijo. In za to rabm čas, pa še dat se mi more.*)
            |_ -> repna acc (tmp ^ " " ^ x) xs 
    in 
    flip (repna [] "" data)


(*pridobi key-se *)
let prep_pass pass =  List.map (fun str_raw -> List.nth (String.split_on_char ':' str_raw) 0) (String.split_on_char ' ' pass) 

let field_to_index (a, b, c, d, e, f, g) = function
    | "byr" -> (a + 1, b, c, d, e, f, g)
    | "iyr" -> (a, b + 1, c, d, e, f, g)
    | "eyr" -> (a, b, c + 1, d, e, f, g)
    | "hgt" -> (a, b, c, d+ 1, e, f, g)
...
let check_field (a, b, c, d, e, f, g)  = function
    | ("byr", value) -> if value >= 1920 && value <= 2002 then (a + 1, b, c, d, e, f, g) else (a, b, c, d, e, f, g)
    | ("iyr", value) -> if value >= 2010 && value <= 2020 then (a + 1, b, c, d, e, f, g) else (a, b, c, d, e, f, g)
    | ("eyr", value) -> if value >= 2020 && value <= 2030 then (a + 1, b, c, d, e, f, g) else (a, b, c, d, e, f, g)
    | ("cm", value) -> if value >= 1920 && value <= 2002 then (a + 1, b, c, d, e, f, g) else (a, b, c, d, e, f, g)
    | ("in", value) -> if value >= 1920 && value <= 2002 then (a + 1, b, c, d, e, f, g) else (a, b, c, d, e, f, g)
    | ("hcl", value) -> if value >= 1920 && value <= 2002 then (a + 1, b, c, d, e, f, g) else (a, b, c, d, e, f, g)
    | ("ecl", value) -> if value >= 1920 && value <= 2002 then (a + 1, b, c, d, e, f, g) else (a, b, c, d, e, f, g)
    | ("pid", value) -> if value >= 1920 && value <= 2002 then (a + 1, b, c, d, e, f, g) else (a, b, c, d, e, f, g)
    | _  -> (a, b, c, d, e, f, g)

let check_pass pass= 
    let rec repna acc = function
        | [] -> acc
        | x::xs -> repna (field_to_index acc x) xs
    in 
    let ans = repna (0,0,0,0,0,0,0) pass in
    if ans = (1,1,1,1,1,1,1) then 1
    else 0

let naloga_1 data = 
    let raw_passes =  get_data (String.split_on_char '\n'  data ) in 
    let rec sestej_ok_passe acc = function
        | [] -> acc 
        | x::xs -> sestej_ok_passe (acc + check_pass ( prep_pass x)) xs 
    in 
    sestej_ok_passe 0 raw_passes



let datasi = "hgt:176cm
iyr:2013
hcl:#fffffd ecl:amb
byr:2000
eyr:2034
